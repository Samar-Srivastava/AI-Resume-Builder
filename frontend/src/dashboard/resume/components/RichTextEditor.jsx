...
function RichTextEditor({onRichTextEditorChange,index,defaultValue}) {
    ...
    const GenerateSummeryFromAI=async()=>{
      ...
      setLoading(true)
      try {
        const promptText=PROMPT.replace('{positionTitle}',resumeInfo.Experience[index].title);
        const resp=await generateWithAI(promptText, { jsonMode: false });
        const cleaned=sanitizeHtml(resp.replace('[','').replace(']',''));
        setValue(cleaned);
